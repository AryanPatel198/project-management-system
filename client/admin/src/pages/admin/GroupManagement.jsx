...
const fetchGroups = async () => {
  try {
    const response = await axios.get(`${API_BASE_URL}/get-groups`, {
      headers,
      params: {
        // send course only if filter is not "All"
        course:
          selectedClassFilter !== "All"
            ? selectedClassFilter.split(" ")[0]
            : undefined,
        // send year only if filter is not "All Years"
        year:
    ...
    if (response.data?.success) {
      ...
      setGroups([]);
      console.warn("No groups found or server returned error.");
    }
  } catch (err) {
